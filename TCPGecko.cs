...
namespace BotwTrainer
{
    ...
    public class Dump
    {
        ...
        private void Construct(UInt32 theStartAddress, UInt32 theEndAddress, int theFileNumber)
        {
            ...
            this.mem = new Byte[this.endAddress - this.startAddress];
        ...
        public UInt32 ReadAddress32(UInt32 addressToRead)
        {
            ...
            if (addressToRead < this.startAddress) return 0;
            if (addressToRead > this.endAddress - 4) return 0;
            Byte[] buffer = new Byte[4];
            Buffer.BlockCopy(this.mem, this.index(addressToRead), buffer, 0, 4);
        ...
        {
            return (int)(addressToRead - this.startAddress);
        }

        public UInt32 ReadAddress(UInt32 addressToRead, int numBytes)
        {
            if (addressToRead < this.startAddress) return 0;
            if (addressToRead > this.endAddress - numBytes) return 0;
            
            Byte[] buffer = new Byte[4];
            Buffer.BlockCopy(this.mem, this.index(addressToRead), buffer, 0, numBytes);
        ...
        public void WriteStreamToDisk()
        {
            ...
            string myFile = myDirectory + "dump" + this.fileNumber.ToString() + ".dmp";

            this.WriteStreamToDisk(myFile);
        }

        public void WriteStreamToDisk(string filepath)
        {
            FileStream foo = new FileStream(filepath, FileMode.Create);
            foo.Write(this.mem, 0, (int)(this.endAddress-this.startAddress));
            foo.Close();
            foo.Dispose();
        }
        ...
        {
            get { return this.readCompletedAddress; }
            set { this.readCompletedAddress = value; }
        }
    ...
    public class TCPGecko
    {
        ...
        public void Upload(UInt32 startupload, UInt32 endupload, Stream sendStream)
        {
            ...
            if (this.GeckoWrite(BitConverter.GetBytes(cmd_upload), 1) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
            ...
            {
                //No output yet availible
                this.SendUpdate(startupload + chunk * packetsize, chunk, allchunks, chunk * packetsize, memlength, retry == 0, false);
                //Set buffer
                buffer = new Byte[uplpacketsize];
                //Read buffer from stream
                sendStream.Read(buffer, 0, (int)uplpacketsize);
                FTDICommand returnvalue = this.GeckoWrite(buffer, (int)uplpacketsize);
                if (returnvalue == FTDICommand.CMD_ResultError)
                {
                    retry++;
                    if (retry >= 3)
            ...
            while (lastchunk > 0)
            {
                ...
                FTDICommand returnvalue = this.GeckoWrite(buffer, (int)lastchunk);
                if (returnvalue == FTDICommand.CMD_ResultError)
                {
                    retry++;
                    if (retry >= 3)
            ...
            if (reply != GCACK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDIInvalidReply);
            this.SendUpdate(endupload, allchunks, allchunks, memlength, memlength, true, false);
        }
        ...
        public void Pause()
        {
            ...
            if (this.RawCommand(cmd_pause) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
        }
        ...
        {
            bool WasRunning = (this.status() == WiiStatus.Running);
            while (WasRunning)
            {
                this.Pause();
                ...
                WasRunning = (this.status() == WiiStatus.Running);
            }
        }
        ...
        public void Resume()
        {
            ...
            if (this.RawCommand(cmd_unfreeze) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
        }
        ...
        public WiiStatus status()
        {
            ...
            //Send status command
            if (this.RawCommand(cmd_status) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
...
//			System.Threading.Thread.Sleep(10);
            ...
            if (this.GeckoRead(buffer, 1) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDIReadDataError);
        ...
        public void Step() 
        {
            ...
            //Send step command
            if (this.RawCommand(cmd_step) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
        }
        ...
        protected void Breakpoint(UInt32 address, Byte bptype, bool exact)
        {
            ...
            if (exact)
                useGeckoBP = (this.VersionRequest() != GCNgcVer);

            if (!useGeckoBP) //classic PPC breakpoint
            {
                if (this.RawCommand(cmd_breakpoint) != FTDICommand.CMD_OK)
                    throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);

                //Convert lowaddr to BigEndian
                UInt32 breakpaddr = ByteSwap.Swap(lowaddr);

                if (this.GeckoWrite(BitConverter.GetBytes(breakpaddr),4)!=FTDICommand.CMD_OK)
                    throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
            }
            else //advanced exact Gecko breakpoint
            {
                if (this.RawCommand(cmd_nbreakpoint) != FTDICommand.CMD_OK)
                    throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);

                UInt64 breakpaddr = ((UInt64)lowaddr) << 32 | ((UInt64)address);
                breakpaddr = ByteSwap.Swap(breakpaddr);

                if (this.GeckoWrite(BitConverter.GetBytes(breakpaddr), 8) != FTDICommand.CMD_OK)
                    throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
            }
        }
        ...
        public void BreakpointX(UInt32 address)
        {
            ...
            if (this.RawCommand(cmd_breakpointx) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);

            //Send address to handler
            if(this.GeckoWrite(BitConverter.GetBytes(baddress),4) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
        }
        ...
        {
            if (this.RawCommand(cmd_cancelbp) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
        }
#endregion
        ...
        public Byte VersionRequest()
        {
            ...
            if (this.RawCommand(cmd_version) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
            ...
            do
            {
                if (this.GeckoRead(buffer, 1) == FTDICommand.CMD_OK)
                {
                    if (this.AllowedVersion(buffer[0]))
                    {
                        result = buffer[0];
                        break;
                    }
        ...
        public void GetRegisters(Stream stream, uint contextAddress) 
        {
            ...
            this.Dump(contextAddress + 8, contextAddress + 8 + bytesExpected, buffer);
        ...
        public void SendRegisters(Stream sendStream, uint contextAddress)
        {
            ...
            this.Upload(contextAddress + 8, contextAddress + 8 + 0x98, buffer);
        }
        #endregion
        ...
        private void insertInto(Stream insertStream, UInt64 value)
        {
            ...
            this.writeInt64(tempstream, value);
        ...
        public void sendCheats(Stream inputStream)
        {
            ...
            if (this.GeckoWrite(BitConverter.GetBytes(cmd_sendcheats), 1) != FTDICommand.CMD_OK)
            {
                cheatStream.Close();
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
            }
            ...
            if (this.GeckoWrite(BitConverter.GetBytes(blength), 4) != FTDICommand.CMD_OK)
            {
                cheatStream.Close();
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
            }
            ...
            {
                //No output yet availible
                this.SendUpdate(0x00d0c0de, chunk, allchunks, chunk * packetsize, length, retry == 0, false);
                //Set buffer
                buffer = new Byte[uplpacketsize];
                //Read buffer from stream
                ...
                if (returnvalue == FTDICommand.CMD_ResultError)
                {
                    ...
                    this.GeckoWrite(BitConverter.GetBytes(GCRETRY), 1);
                    continue;
                }
                else if (returnvalue == FTDICommand.CMD_FatalError)
                {
                ...
                Byte[] response = new Byte[1];
                returnvalue = this.GeckoRead(response, 1);
            ...
            while (lastchunk > 0)
            {
                ...
                FTDICommand returnvalue = this.GeckoWrite(buffer, (Int32)lastchunk);
                if (returnvalue == FTDICommand.CMD_ResultError)
                {
                    retry++;
                    if (retry >= 3)
            ...
            this.SendUpdate(0x00d0c0de, allchunks, allchunks, length, length, true,false);
            cheatStream.Close();
        }
        ...
        {
            if (this.RawCommand(cmd_cheatexec) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
        }
        #endregion
        ...
        public void Hook(bool pause, WiiLanguage language, WiiPatches patches, WiiHookType hookType)
        {
            ...
            if (this.RawCommand(command) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
        ...
        {
            return (UInt32)(this.rpc64(address, args) >> 32);
        }
        ...
        public UInt64 rpc64(UInt32 address, params UInt32[] args)
        {
            ...
            //Send read
            if (this.RawCommand(cmd_rpc) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
            

            //write value
            if (this.GeckoWrite(buffer, buffer.Length) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);

            if (this.GeckoRead(buffer, 8) != FTDICommand.CMD_OK)
                throw new ETCPGeckoException(ETCPErrorCode.FTDICommandSendError);
